...
from ..dataset.dataset import InquiryDataset
from official.nlp import optimization  # to create AdamW optimizer
import keras
import tensorflow_hub as hub
from ..inquiry_estimator import InquiryAnalyzerAssistant
...
class InquiryAnalyzerBERTModel(keras.Model):
    ...
    def __init__(self, tfhub_handle_preprocess: str, tfhub_handle_encoder: str, *args, **kwargs):
        ...
        self.preprocess_layer = hub.KerasLayer(
            tfhub_handle_preprocess, name="PREPROCESS")
        self.encoder_layer = hub.KerasLayer(
            tfhub_handle_encoder, trainable=True, name="BERT_ENCODER")
        self.dropout = keras.layers.Dropout(0.1, name="DROPOUT")
        self.dense = keras.layers.Dense(
            10, activation="softmax", name="FINAL_CLASSIFIER")
    ...
    def from_file(filepath, ds_path, epochs=100):
        optimizer = InquiryAnalyzerBERT.get_optimizer(InquiryAnalyzerDatasetManagerBERT.get_ds(32, ds_path), epochs)
        model = keras.models.load_model(filepath,
                                        custom_objects={"AdamWeightDecay":
                                                            optimizer})
        return model
...
class InquiryAnalyzerBERT(InquiryAnalyzer):
    ...
    def classify(self, a: list or str) -> Tuple[str, np.ndarray]:
        ...
        result_tensor = self.model.call(
            tf.convert_to_tensor(tf.convert_to_tensor(a)))
        result_np = result_tensor.numpy()

        return InquiryAnalyzerAssistant.classifierstringar(result_np.round()), result_np
...
class InquiryAnalyzerDatasetManagerBERT:
    ...
    def get_ds(BATCH_SIZE, dataset_path: str) -> tf.data.Dataset:
        ...
        # tensorflow can't convert labels easily
        train_labels = train_labels.reshape((npdataset[:, 1].shape[0], 10))
        train_labels = tf.convert_to_tensor(train_labels)
        dataset = tf.data.Dataset.from_tensor_slices(
            (train_examples, train_labels))
        train_ds = dataset.batch(BATCH_SIZE, drop_remainder=False).cache().prefetch(
